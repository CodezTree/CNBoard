...
public class EnrolmentSubject extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Subject_spin.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int position, long id) {
                ...
                Log.d("디버그,arrKeyS.0",String.valueOf(arrKeyS.get(0)));
                Log.d("디버그,arrKeyS.1",String.valueOf(arrKeyS.get(1)));
    ...
    protected void getData(){ // 앱 껏다가 켰을 때 자기가 저장한 시험범위 리스트를 잃어버리지 않도록 저장해주는 역할
        ...
        for ( MyGradeNcode data : mySubject) {
            ...
            Log.d("마지막 디버그",String.valueOf(data.myCode));
            Toast.makeText(getApplicationContext(), "마지막!", Toast.LENGTH_SHORT).show();
            Toast.makeText(getApplicationContext(), String.valueOf(data.myCode), Toast.LENGTH_SHORT).show();

        /*for (int i =0; i<= mySubject.size();i++){
                 MyGradeNcode temp = mySubject.get(i);
                 Log.d("디버그",String.valueOf(temp.myCode));
                 Log.d("디버그",String.valueOf(temp.myCode));
                 grade = temp.myGrade;
                 arrKey1 = temp.myCode;
            onListItemAdd(grade,arrKey1);*/
